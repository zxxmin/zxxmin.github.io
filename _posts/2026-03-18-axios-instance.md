...
# 개요
Web 백엔드, AI 백엔드, Data 백엔드 등 여러 백엔드가 서비스와 연동하는 경우, API 요청마다 baseURL을 변경해야하는 문제 발생.

당시 AI 백엔드 호출 방법과 Web 백엔드 호출 방법에 대한 차이도 있었고, 인증/인가 또는 menuId key 값을 보내는 설정이 Web 백엔드만 설계가 되어있어서 AI 백엔드 부분에는 불필요한 호출을 막기 위해 설계 진행
